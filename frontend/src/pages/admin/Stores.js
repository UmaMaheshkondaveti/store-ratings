...
const AdminStores = () => {
  ...
  const handleAddStore = async (e) => {
    ...
    try {
      ...
      setFormErrors({
        general: error.response?.data?.message || "Failed to add store",
      })
    }
  }
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {store.average_rating ? (
                      <div className="flex items-center">
                        <StarRating rating={Math.round(store.average_rating)} readOnly />
                        <span className="ml-2">{Number.parseFloat(store.average_rating).toFixed(1)}</span>
                      </div>
                    ) : (
                      "No ratings yet"
                    )}
